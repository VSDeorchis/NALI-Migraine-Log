//
//  MacMigraineRiskView.swift
//  NALI Migraine Log macOS
//
//  Risk prediction dashboard for macOS.
//  Adapted from the iOS MigraineRiskView with macOS-compatible colors and layout.
//

import SwiftUI
import Charts

struct MacMigraineRiskView: View {
            ...
            ForEach(factors) { factor in
                HStack(spacing: 12) {
                    ...
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 3)
                            .fill(factor.color)
                            .frame(width: min(50, 50 * factor.contribution / 0.30), height: 8)
                    }
                }
                .padding(.vertical, 4)
    ...
    private func refreshPrediction() async {
        ...
        if let coords = LocationManager.shared.currentCoordinates {
            do {
                ...
                AppLogger.weather.error("Forecast fetch failed: \(error.localizedDescription, privacy: .private)")
            }
        }
