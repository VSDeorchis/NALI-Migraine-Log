...
│   ├── Views/, ViewModels/, Utilities/      # iOS-specific UI (incl. FeedbackFormView, #if os(iOS))
...
| **Review prompt** | `Shared/Services/ReviewPromptCoordinator.swift` | Pre-filter for Apple's native `requestReview()` sheet ("Enjoying Headway? / Tap a star to rate it on the App Store."). Tracks tenure (≥7 days), entries logged (≥5), and a 180-day cooldown in `UserDefaults` so we don't burn the user's annual quota on a brand-new install. No custom pre-prompt UI — we call `requestReview()` directly from `MigraineLogView`. macOS uses an unconditional "Rate" menu command instead. |
| **In-app feedback** | iOS `Views/FeedbackFormView.swift` | Always-available "Send Feedback" entry point from Settings → Help & Feedback: category + star + free-text form, then hands off to `MFMailComposeViewController` (clipboard fallback if mail isn't configured). All iOS-only — guarded by `#if os(iOS)` so the file is a no-op on macOS/watchOS targets. |
...
| **iPad layouts (in-destination)** | `Views/MigraineLogView.swift`, `Views/CalendarView.swift`, `Views/StatisticsView.swift` | Each destination branches on `horizontalSizeClass` to render a wide-canvas layout when shown in the iPad detail column: `MigraineLogView` becomes a 380 pt master list + inline `MigraineDetailView` (the iPhone sheet pattern is bypassed on regular size class via `iPadBody`), `CalendarView` shows the month grid alongside a day or month migraine list with a `selectedDay` selection ring on `DayCell`, and `StatisticsView` switches its KPI grid from 2 columns to 4 and caps the dashboard at `maxWidth: 1100` for readability. Combined with the outer sidebar this produces a 3-column sidebar → list → detail layout for the Log tab without nesting `NavigationSplitView` (which has known toolbar quirks). |
| **iPad affordances** | `iOSContentView.swift`, `Views/MigraineLogView.swift`, `Views/CalendarView.swift`, `Views/Analytics/HealthCorrelationsSectionView.swift` | Hardware-keyboard and trackpad polish for iPad / Magic-Keyboard users: `iOSContentView.globalKeyboardShortcuts` hosts off-screen buttons that bind ⌘N (log new migraine) and ⌘1 / ⌘2 / ⌘3 / ⌘4 (jump to Log / Calendar / Analytics / About) — kept at the root so they fire from any destination, with the per-destination character coming from `AppDestination.shortcutKey`. The iPad sidebar gets a docked "Log Migraine" CTA via `.safeAreaInset(edge: .bottom)` plus a redundant `+` in the sidebar toolbar; immediately above the CTA, `sidebarStatsSummary` prints "X this month" and "Y days migraine-free" using the same `currentMigraineFreeStreak()` extension the Analytics dashboard uses, so the sidebar feels like a dashboard rather than a pure nav list. Trackpad hover effects use `.hoverEffect(.highlight)` on row-style surfaces (migraine rows, day cells, filter chips) and `.hoverEffect(.lift)` on card-style surfaces (KPI tiles via `tileLink`, sleep/HRV/cycle correlation cards). |
| **iPad Settings two-pane** | `Views/SettingsView.swift` | `SettingsView` branches on `horizontalSizeClass` to render a `NavigationSplitView` with a category sidebar + per-category detail form on iPad, instead of the long single-form scroll users get on iPhone. Categories live in the `SettingsCategory` enum (one case per existing `*Section`, plus title / SF Symbol / tint), and `iPadDetailContent(for:)` wraps the matching section in its own `Form` so the styling stays identical. Adding a new section: append a case to `SettingsCategory`, route it in `iPadDetailContent`, and append it to the `iPhoneBody` Form. The `recovery` case is gated on `recoveryFileURL != nil` via `visibleCategories` so users without a corrupt-store backup don't see a stub entry. |
| **iPad notes editor** | `Views/NewMigraineView.swift`, `Views/MigraineDetailView.swift` | The notes `TextEditor` doubles its `minHeight` (100 → 200 pt) when `horizontalSizeClass == .regular` so Apple Pencil scribble has enough canvas to land a sentence or two before the field starts scrolling. Scribble itself is on by default for any SwiftUI `TextEditor` — there's no opt-in modifier — so the only real polish is the larger writable surface plus a size-class-aware accessibility hint ("Type with the keyboard or write with Apple Pencil"). |
...
- **iPad in-destination layouts**: when a destination has a master/detail relationship (Log: list → entry; Calendar: month grid → day; eventually Analytics: KPI grid → drill-down) prefer an `HStack { master; Divider(); detail }` inside the destination, gated on `horizontalSizeClass == .regular`. Don't nest `NavigationSplitView` inside the outer one — it works but produces double toolbars and unpredictable sidebar collapse. The current pattern (see `MigraineLogView.iPadBody` / `CalendarView.iPadBody`) keeps full control over column widths, leaves the outer sidebar untouched, and reuses the same shared chrome (`masterColumnContent`, `monthSelector`, `calendarGrid`) on both the compact and regular paths so the two layouts never drift.
- **Keyboard shortcuts**: register hardware-keyboard shortcuts at the root (`iOSContentView.globalKeyboardShortcuts`) using off-screen buttons (`opacity(0)` + zero-frame), not on the visible toolbar buttons. Toolbar buttons only exist when their tab is on screen, so attaching ⌘N to `MigraineLogView`'s `+` would silently break the shortcut on Calendar/Analytics. The off-screen pattern also makes the iPad's "press and hold ⌘" overlay surface every shortcut consistently. Adding a new shortcut: append a `Button` inside `globalKeyboardShortcuts` (or, for tab navigation, a new case in `AppDestination.shortcutKey`).
- **Hover effects**: use `.hoverEffect(.highlight)` for row-style affordances (list rows, day cells, filter chips) and `.hoverEffect(.lift)` for card-style affordances (KPI tiles, correlation cards). Always pair `.hoverEffect()` with a `.contentShape(...)` whose shape matches the visible silhouette so the hover region tracks the design (`Capsule()` for chips, `RoundedRectangle(...)` for cards) instead of the hit-test rectangle. No-ops on iPhone touch input — they only activate for trackpad / Magic-Keyboard pointer.
- **iPad two-pane Settings**: when a long settings form crosses ~6 sections, branch on `horizontalSizeClass` and split into a `NavigationSplitView` sidebar + detail. Keep the iPhone Form as a fallback (`iPhoneBody`) so we don't duplicate section logic; the iPad detail just routes a category enum through a `switch` to the right `Section` view, wrapped in its own `Form`. The same pattern works for any future hub-style screen (e.g. Analytics → category drill-down) without nesting `NavigationSplitView` inside the outer one.
- **Apple Pencil scribble**: SwiftUI `TextEditor` and `TextField` accept Scribble input automatically — no opt-in modifier exists. The only iPad polish that actually matters is giving the editor enough vertical canvas to land a handwritten sentence (current convention: `minHeight: horizontalSizeClass == .regular ? 200 : 100`). Don't try to overlay a custom UITextView wrapper to "enable" Scribble — that strictly makes things worse by losing the SwiftUI focus state.
...
- **In-app review & feedback flow**: Apple's `requestReview()` is called directly from `MigraineLogView` after `ReviewPromptCoordinator.shouldShowReviewPrompt` says the gate is open (tenure ≥7 days, entries ≥5 logged, 180-day cooldown). Users see Apple's standard sheet — "Enjoying Headway? / Tap a star to rate it on the App Store." — not a custom pre-prompt. Users who want to send unsolicited feedback hit Settings → "Send Feedback" which opens `FeedbackFormView` (in-app form, sends via the user's mail client to `support@cicgconsulting.com`). Constants for the gate are tunable at the top of `ReviewPromptCoordinator`. `FeedbackFormView` is wrapped in `#if os(iOS)` because it depends on `UIKit`/`MessageUI`; macOS gets straight-shot menu commands instead.
