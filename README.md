...
| **Watch ↔ iPhone sync** | `Shared/WatchConnectivityManager.swift`, `Shared/Models/Sync/` | Protocol v2: each side sends *deltas* of its own edits — over `sendMessage` while the counterpart is reachable (the receiver replies only after saving), falling back to `transferUserInfo` (queued offline); dirty ids are cleared only on acknowledgement; the phone also publishes a compact *snapshot* through `updateApplicationContext`; deletions travel as tombstones (90-day retention). Conflicts resolve last-writer-wins on `modifiedAt`. Risk travels as a validated `WatchRiskPayload` (`riskUpdateV2`); the Watch persists the last adopted score and shows an explicit "no score yet" state instead of a fake 0%. |
